...
func QueryAmData(client sbi.ConsumerClient, ueId string, servingPlmnId string, fields []string, supportedFeatures string, ifNoneMatch string, ifModifiedSince string) (result models.AccessAndMobilitySubscriptionData, err error) {
	...
	req.Path = fmt.Sprintf("%s/subscription-data/{ueId}/{servingPlmnId}/provisioned-data/am-data", ServicePath)
...
func OnQueryAmData(ctx sbi.RequestContext, handler interface{}) (resp sbi.Response) {
	prod := handler.(AccessAndMobilitySubscriptionDataDocumentApiHandler)
...
type AccessAndMobilitySubscriptionDataDocumentApiHandler interface {
