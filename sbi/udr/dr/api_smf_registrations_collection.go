...
func QuerySmfRegList(client sbi.ConsumerClient, ueId string, supportedFeatures string) (result []models.SmfRegistration, err error) {
	...
	req.Path = fmt.Sprintf("%s/subscription-data/{ueId}/context-data/smf-registrations", ServicePath)
...
func OnQuerySmfRegList(ctx sbi.RequestContext, handler interface{}) (resp sbi.Response) {
	prod := handler.(SMFRegistrationsCollectionApiHandler)
...
type SMFRegistrationsCollectionApiHandler interface {
