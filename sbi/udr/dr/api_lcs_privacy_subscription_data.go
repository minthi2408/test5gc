...
func QueryLcsPrivacyData(client sbi.ConsumerClient, ueId string, fields []string, supportedFeatures string, ifNoneMatch string, ifModifiedSince string) (result models.LcsPrivacyData, err error) {
	...
	req.Path = fmt.Sprintf("%s/subscription-data/{ueId}/lcs-privacy-data", ServicePath)
...
func OnQueryLcsPrivacyData(ctx sbi.RequestContext, handler interface{}) (resp sbi.Response) {
	prod := handler.(LCSPrivacySubscriptionDataApiHandler)
...
type LCSPrivacySubscriptionDataApiHandler interface {
