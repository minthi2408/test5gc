...
func QueryLcsMoData(client sbi.ConsumerClient, ueId string, fields []string, supportedFeatures string, ifNoneMatch string, ifModifiedSince string) (result models.LcsMoData, err error) {
	...
	req.Path = fmt.Sprintf("%s/subscription-data/{ueId}/lcs-mo-data", ServicePath)
...
func OnQueryLcsMoData(ctx sbi.RequestContext, handler interface{}) (resp sbi.Response) {
	prod := handler.(LCSMobileOriginatedSubscriptionDataApiHandler)
...
type LCSMobileOriginatedSubscriptionDataApiHandler interface {
